...
/**
 *   	\file       dolifarmagrodrug_list.php
 *		\ingroup    dolifarm
 *		\brief      List page for dolifarmagrodrug
 */
...
include_once __DIR__.'/class/dolifarmagrodrug.class.php';
// for other modules
...
if ($enablepermissioncheck) {
	...
	$permissiontoadd = $user->hasRight('dolifarm', 'dolifarmagrodrug', 'write');
	$permissiontodelete = $user->hasRight('dolifarm', 'dolifarmagrodrug', 'delete');
} else {
	$permissiontoread = 1;
	$permissiontoadd = 1;
...
if (empty($reshook)) {
	...
	// Mass actions
	$objectclass = 'DoliFarmAgroDrug';
	$objectlabel = 'DoliFarmAgroDrug';
...
$title = $langs->trans("DoliFarmAgroDrugs");
//$help_url = "EN:Module_DoliFarmAgroDrug|FR:Module_DoliFarmAgroDrug_FR|ES:Módulo_DoliFarmAgroDrug";
$help_url = '';
$morejs = array();
$morecss = array();
...
if ($num == 1 && getDolGlobalInt('MAIN_SEARCH_DIRECT_OPEN_IF_ONLY_ONE') && $search_all && !$page) {
	...
	$id = $obj->rowid;
	header("Location: ".dol_buildpath('/dolifarm/dolifarmagrodrug_card.php', 1).'?id='.((int) $id));
	exit;
}
...
$newcardbutton .= dolGetButtonTitle($langs->trans('New'), '', 'fa fa-plus-circle', dol_buildpath('/dolifarm/dolifarmagrodrug_card.php', 1).'?action=create&backtopage='.urlencode($_SERVER['PHP_SELF']), '', $permissiontoadd);

print_barre_liste($title, $page, $_SERVER["PHP_SELF"], $param, $sortfield, $sortorder, $massactionbutton, $num, $nbtotalofrecords, $object->picto, 0, $newcardbutton, '', $limit, 0, 0, 1);

// Add code for pre mass action (confirmation or email presend form)
$topicmail = "SendDoliFarmAgroDrugRef";
$modelmail = "dolifarmagrodrug";
$objecttmp = new DoliFarmAgroDrug($db);
