...
/**
 * \file        class/dolifarmestimatecosts.class.php
 * \ingroup     dolifarm
 * \brief       This file is a CRUD class file for DoliFarmEstimateCosts (Create/Read/Update/Delete)
 */
...
/**
 * Class for DoliFarmEstimateCosts
 */
class DoliFarmEstimateCosts extends CommonObject
{
	/**
	 ...
	 */
	public $element = 'dolifarmestimatecosts';
	 ...
	 */
	public $table_element = 'dolifarm_dolifarmestimatecosts';

	/**
	 * @var string 		If permission must be checked with hasRight('dolifarm', 'read') and not hasright('dolifarm', 'dolifarmestimatecosts', 'read'), you can uncomment this line
	 */
	//public $element_for_permission = 'dolifarm';

	/**
	 * @var string 		String with name of icon for dolifarmestimatecosts. Must be a 'fa-xxx' fontawesome code (or 'fa-xxx_fa_color_size') or 'dolifarmestimatecosts@dolifarm' if picto is file 'img/object_dolifarmestimatecosts.png'.
	 */
	...
	public $fields = array(
		...
		"fk_operation_type" => array("type" => "varchar(32)", "label" => "Fkoperationtype", "enabled" => "1", 'position' => 20, 'notnull' => 1, "visible" => "-1", "css" => "maxwidth500 widthcentpercentminusxx",),
		"fk_soc" => array("type" => "integer:Societe:societe/class/societe.class.php", "label" => "ThirdParty", "picto" => "company", "enabled" => "1", 'position' => 25, 'notnull' => 0, "visible" => "-1", "css" => "maxwidth500 widthcentpercentminusxx", "csslist" => "tdoverflowmax150",),
		"duration_min_ha" => array("type" => "int", "label" => "Durationminha", "enabled" => "1", 'position' => 30, 'notnull' => 0, "visible" => "-1",),
		"cost_std_ha" => array("type" => "double(24,8)", "label" => "Coststdha", "enabled" => "1", 'position' => 35, 'notnull' => 0, "visible" => "-1",),
		...
		"tms" => array("type" => "timestamp", "label" => "DateModification", "enabled" => "1", 'position' => 55, 'notnull' => 0, "visible" => "-1",),
		"fk_user_creat" => array("type" => "integer:User:user/class/user.class.php", "label" => "UserAuthor", "picto" => "user", "enabled" => "1", 'position' => 60, 'notnull' => 0, "visible" => "-2", "css" => "maxwidth500 widthcentpercentminusxx", "csslist" => "tdoverflowmax150",),
		"fk_user_modif" => array("type" => "integer:User:user/class/user.class.php", "label" => "UserModif", "picto" => "user", "enabled" => "1", 'position' => 65, 'notnull' => -1, "visible" => "-2", "css" => "maxwidth500 widthcentpercentminusxx", "csslist" => "tdoverflowmax150",),
		"import_key" => array("type" => "varchar(14)", "label" => "ImportId", "enabled" => "1", 'position' => 900, 'notnull' => 0, "visible" => "-2",),
	);
	public $rowid;
	public $fk_operation_type;
	public $fk_soc;
	public $duration_min_ha;
	public $cost_std_ha;
	public $label;
	public $note_public;
	...
	//  */
	// public $table_element_line = 'dolifarm_dolifarmestimatecostsline';
	...
	//  */
	// public $fk_element = 'fk_dolifarmestimatecosts';

	// /**
	//  * @var string    Name of subtable class that manage subtable lines
	//  */
	// public $class_element_line = 'DoliFarmEstimateCostsline';

	// /**
	//  * @var array	List of child tables. To test if we can delete object.
	//  */
	// protected $childtables = array('mychildtable' => array('name'=>'DoliFarmEstimateCosts', 'fk_element'=>'fk_dolifarmestimatecosts'));
	...
	//  */
	// protected $childtablesoncascade = array('dolifarm_dolifarmestimatecostsdet');

	// /**
	//  * @var DoliFarmEstimateCostsLine[]     Array of subtable lines
	//  */
	// public $lines = array();
	...
	public function __construct(DoliDB $db)
	{
		...
		$this->picto = dolifarm_get_fa_icon('estimatecosts');
		...
		/*if ($user->hasRight('dolifarm', 'dolifarmestimatecosts', 'read')) {
	...
	public function validate($user, $notrigger = 0)
	{
		...
		/* if (! ((!getDolGlobalInt('MAIN_USE_ADVANCED_PERMS') && $user->hasRight('dolifarm', 'dolifarmestimatecosts', 'write'))
		 || (getDolGlobalInt('MAIN_USE_ADVANCED_PERMS') && $user->hasRight('dolifarm', 'dolifarmestimatecosts_advance', 'validate')))
		 {
		...
		if (!$error) {
			...
			if (preg_match('/^[\(]?PROV/i', $this->ref)) {
				...
				$sql = 'UPDATE '.$this->db->prefix()."ecm_files set filename = CONCAT('".$this->db->escape($this->newref)."', SUBSTR(filename, ".(strlen($this->ref) + 1).")), filepath = 'dolifarmestimatecosts/".$this->db->escape($this->newref)."'";
				$sql .= " WHERE filename LIKE '".$this->db->escape($this->ref)."%' AND filepath = 'dolifarmestimatecosts/".$this->db->escape($this->ref)."' and entity = ".$conf->entity;
				$resql = $this->db->query($sql);
				if (!$resql) {
					$error++;
					$this->error = $this->db->lasterror();
				}
				$sql = 'UPDATE '.$this->db->prefix()."ecm_files set filepath = 'dolifarmestimatecosts/".$this->db->escape($this->newref)."'";
				$sql .= " WHERE filepath = 'dolifarmestimatecosts/".$this->db->escape($this->ref)."' and entity = ".$conf->entity;
				$resql = $this->db->query($sql);
				if (!$resql) {
					$error++;
					$this->error = $this->db->lasterror();
				...
				$dirsource = $conf->dolifarm->dir_output.'/dolifarmestimatecosts/'.$oldref;
				$dirdest = $conf->dolifarm->dir_output.'/dolifarmestimatecosts/'.$newref;
				if (!$error && file_exists($dirsource)) {
					...
					if (@rename($dirsource, $dirdest)) {
						...
						$listoffiles = dol_dir_list($conf->dolifarm->dir_output.'/dolifarmestimatecosts/'.$newref, 'files', 1, '^'.preg_quote($oldref, '/'));
	...
	public function getTooltipContentArray($params)
	{
		...
		if (getDolGlobalInt('MAIN_OPTIMIZEFORTEXTBROWSER')) {
			return ['optimize' => $langs->trans("ShowDoliFarmEstimateCosts")];
		}
		$datas['picto'] = img_picto('', $this->picto).' <u>'.$langs->trans("DoliFarmEstimateCosts").'</u>';
	...
	public function getNomUrl($withpicto = 0, $option = '', $notooltip = 0, $morecss = '', $save_lastsearch_value = -1)
	{
		...
		$baseurl = dol_buildpath('/dolifarm/dolifarmestimatecosts_card.php', 1);
		$query = ['id' => $this->id];
		if ($option !== 'nolink') {
		...
		if (empty($notooltip)) {
			if (getDolGlobalInt('MAIN_OPTIMIZEFORTEXTBROWSER')) {
				$label = $langs->trans("ShowDoliFarmEstimateCosts");
	...
	public function getLinesArray()
	{
		...
		$objectline = new DoliFarmEstimateCostsLine($this->db);
		$result = $objectline->fetchAll('ASC', 'position', 0, 0, '(fk_dolifarmestimatecosts:=:'.((int) $this->id).')');
	...
	public function getNextNumRef()
	{
		...
		if (!getDolGlobalString('DOLIFARM_MYOBJECT_ADDON')) {
			$conf->global->DOLIFARM_MYOBJECT_ADDON = 'mod_dolifarmestimatecosts_standard';
		}
		...
		if (getDolGlobalString('DOLIFARM_MYOBJECT_ADDON')) {
			...
			if (class_exists($classname)) {
				$obj = new $classname();
				'@phan-var-force ModeleNumRefDoliFarmEstimateCosts $obj';
	...
	public function generateDocument($modele, $outputlangs, $hidedetails = 0, $hidedesc = 0, $hideref = 0, $moreparams = null)
	{
		...
		if (!dol_strlen($modele)) {
			if (!empty($this->model_pdf)) {
				...
				$modele = getDolGlobalString('MYOBJECT_ADDON_PDF', 'standard_dolifarmestimatecosts');
			}
		}
...
/**
 * Class DoliFarmEstimateCostsLine. You can also remove this and generate a CRUD class for lines objects.
 */
class DoliFarmEstimateCostsLine extends CommonObjectLine
{
	// To complete with content of an object DoliFarmEstimateCostsLine
	// We should have a field rowid, fk_dolifarmestimatecosts and position

	/**
	 * To overload
	 * @see CommonObjectLine
	 */
	public $parent_element = '';		// Example: '' or 'dolifarmestimatecosts'

	/**
	 * To overload
	 * @see CommonObjectLine
	 */
	public $fk_parent_attribute = '';	// Example: '' or 'fk_dolifarmestimatecosts'
