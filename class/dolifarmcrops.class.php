...
/**
 * \file        class/dolifarmcrops.class.php
 * \ingroup     dolifarm
 * \brief       This file is a CRUD class file for DoliFarmCrops (Create/Read/Update/Delete)
 */
...
/**
 * Class for DoliFarmCrops
 */
class DoliFarmCrops extends CommonObject
{
	/**
	...
	/**
	 * @var string 		If permission must be checked with hasRight('dolifarm', 'read') and not hasright('dolifarm', 'dolifarmcrops', 'read'), you can uncomment this line
	 */
	//public $element_for_permission = 'dolifarm';

	/**
	 * @var string 		String with name of icon for dolifarmcrops. Must be a 'fa-xxx' fontawesome code (or 'fa-xxx_fa_color_size') or 'dolifarmcrops@dolifarm' if picto is file 'img/object_dolifarmcrops.png'.
	 */
	...
	public $fields = array(
		...
		"scientific_name" => array("type" => "varchar(255)", "label" => "Scientificname", "enabled" => "1", 'position' => 30, 'notnull' => 0, "visible" => "-1",),
		"family" => array("type" => "varchar(128)", "label" => "Family", "enabled" => "1", 'position' => 35, 'notnull' => 0, "visible" => "-1",),
		"variety" => array("type" => "varchar(128)", "label" => "Variety", "enabled" => "1", 'position' => 40, 'notnull' => 0, "visible" => "-1",),
		"crop_type" => array("type" => "varchar(32)", "label" => "Croptype", "enabled" => "1", 'position' => 45, 'notnull' => 0, "visible" => "-1",),
		"planting_unit" => array("type" => "varchar(32)", "label" => "Plantingunit", "enabled" => "1", 'position' => 50, 'notnull' => 0, "visible" => "-1",),
		"estimated_yield_std" => array("type" => "double(24,8)", "label" => "Estimatedyieldstd", "enabled" => "1", 'position' => 55, 'notnull' => 0, "visible" => "-1",),
		"status" => array("type" => "int", "label" => "Status", "enabled" => "1", 'position' => 500, 'notnull' => 0, "visible" => "-1",),
		"note_private" => array("type" => "text", "label" => "NotePublic", "enabled" => "1", 'position' => 65, 'notnull' => 0, "visible" => "0", "cssview" => "wordbreak",),
		"note_public" => array("type" => "text", "label" => "NotePrivate", "enabled" => "1", 'position' => 70, 'notnull' => 0, "visible" => "0", "cssview" => "wordbreak",),
		"date_creation" => array("type" => "datetime", "label" => "DateCreation", "enabled" => "1", 'position' => 75, 'notnull' => 0, "visible" => "-1",),
		"tms" => array("type" => "timestamp", "label" => "DateModification", "enabled" => "1", 'position' => 80, 'notnull' => 0, "visible" => "-1",),
		...
		"fk_user_modif" => array("type" => "integer:User:user/class/user.class.php", "label" => "UserModif", "picto" => "user", "enabled" => "1", 'position' => 90, 'notnull' => -1, "visible" => "-2", "css" => "maxwidth500 widthcentpercentminusxx", "csslist" => "tdoverflowmax150",),
	...
	public $variety;
	public $crop_type;
	public $planting_unit;
	public $estimated_yield_std;
	public $status;
	public $note_private;
	public $note_public;
	...
	//  */
	// public $table_element_line = 'dolifarm_cropsline';
	...
	//  */
	// public $fk_element = 'fk_dolifarmcrops';

	// /**
	//  * @var string    Name of subtable class that manage subtable lines
	//  */
	// public $class_element_line = 'DoliFarmCropsline';

	// /**
	//  * @var array	List of child tables. To test if we can delete object.
	//  */
	// protected $childtables = array('mychildtable' => array('name'=>'DoliFarmCrops', 'fk_element'=>'fk_dolifarmcrops'));
	...
	//  */
	// protected $childtablesoncascade = array('dolifarm_cropsdet');

	// /**
	//  * @var DoliFarmCropsLine[]     Array of subtable lines
	//  */
	// public $lines = array();
	...
	public function __construct(DoliDB $db)
	{
		...
		$this->picto = dolifarm_get_fa_icon('crops');
		...
		/*if ($user->hasRight('dolifarm', 'dolifarmcrops', 'read')) {
	...
	public function validate($user, $notrigger = 0)
	{
		...
		/* if (! ((!getDolGlobalInt('MAIN_USE_ADVANCED_PERMS') && $user->hasRight('dolifarm', 'dolifarmcrops', 'write'))
		 || (getDolGlobalInt('MAIN_USE_ADVANCED_PERMS') && $user->hasRight('dolifarm', 'dolifarmcrops_advance', 'validate')))
		 {
		...
		if (!$error) {
			...
			if (preg_match('/^[\(]?PROV/i', $this->ref)) {
				...
				$sql = 'UPDATE '.$this->db->prefix()."ecm_files set filename = CONCAT('".$this->db->escape($this->newref)."', SUBSTR(filename, ".(strlen($this->ref) + 1).")), filepath = 'dolifarmcrops/".$this->db->escape($this->newref)."'";
				$sql .= " WHERE filename LIKE '".$this->db->escape($this->ref)."%' AND filepath = 'dolifarmcrops/".$this->db->escape($this->ref)."' and entity = ".$conf->entity;
				$resql = $this->db->query($sql);
				if (!$resql) {
					$error++;
					$this->error = $this->db->lasterror();
				}
				$sql = 'UPDATE '.$this->db->prefix()."ecm_files set filepath = 'dolifarmcrops/".$this->db->escape($this->newref)."'";
				$sql .= " WHERE filepath = 'dolifarmcrops/".$this->db->escape($this->ref)."' and entity = ".$conf->entity;
				$resql = $this->db->query($sql);
				if (!$resql) {
					$error++;
					$this->error = $this->db->lasterror();
				...
				$dirsource = $conf->dolifarm->dir_output.'/dolifarmcrops/'.$oldref;
				$dirdest = $conf->dolifarm->dir_output.'/dolifarmcrops/'.$newref;
				if (!$error && file_exists($dirsource)) {
					...
					if (@rename($dirsource, $dirdest)) {
						...
						$listoffiles = dol_dir_list($conf->dolifarm->dir_output.'/dolifarmcrops/'.$newref, 'files', 1, '^'.preg_quote($oldref, '/'));
	...
	public function getTooltipContentArray($params)
	{
		...
		if (getDolGlobalInt('MAIN_OPTIMIZEFORTEXTBROWSER')) {
			return ['optimize' => $langs->trans("ShowDoliFarmCrops")];
		}
		$datas['picto'] = img_picto('', $this->picto).' <u>'.$langs->trans("DoliFarmCrops").'</u>';
	...
	public function getNomUrl($withpicto = 0, $option = '', $notooltip = 0, $morecss = '', $save_lastsearch_value = -1)
	{
		...
		$baseurl = dol_buildpath('/dolifarm/dolifarmcrops_card.php', 1);
		$query = ['id' => $this->id];
		if ($option !== 'nolink') {
		...
		if (empty($notooltip)) {
			if (getDolGlobalInt('MAIN_OPTIMIZEFORTEXTBROWSER')) {
				$label = $langs->trans("ShowDoliFarmCrops");
	...
	public function getLinesArray()
	{
		...
		$objectline = new DoliFarmCropsLine($this->db);
		$result = $objectline->fetchAll('ASC', 'position', 0, 0, '(fk_dolifarmcrops:=:'.((int) $this->id).')');
	...
	public function getNextNumRef()
	{
		...
		if (!getDolGlobalString('DOLIFARM_MYOBJECT_ADDON')) {
			$conf->global->DOLIFARM_MYOBJECT_ADDON = 'mod_dolifarmcrops_standard';
		}
		...
		if (getDolGlobalString('DOLIFARM_MYOBJECT_ADDON')) {
			...
			if (class_exists($classname)) {
				$obj = new $classname();
				'@phan-var-force ModeleNumRefDoliFarmCrops $obj';
	...
	public function generateDocument($modele, $outputlangs, $hidedetails = 0, $hidedesc = 0, $hideref = 0, $moreparams = null)
	{
		...
		if (!dol_strlen($modele)) {
			if (!empty($this->model_pdf)) {
				...
				$modele = getDolGlobalString('MYOBJECT_ADDON_PDF', 'standard_dolifarmcrops');
			}
		}
...
/**
 * Class DoliFarmCropsLine. You can also remove this and generate a CRUD class for lines objects.
 */
class DoliFarmCropsLine extends CommonObjectLine
{
	// To complete with content of an object DoliFarmCropsLine
	// We should have a field rowid, fk_dolifarmcrops and position

	/**
	 * To overload
	 * @see CommonObjectLine
	 */
	public $parent_element = '';		// Example: '' or 'dolifarmcrops'

	/**
	 * To overload
	 * @see CommonObjectLine
	 */
	public $fk_parent_attribute = '';	// Example: '' or 'fk_dolifarmcrops'
