...
/**
 * \file        class/dolifarmfarmdossier.class.php
 * \ingroup     dolifarm
 * \brief       This file is a CRUD class file for DoliFarmFarmDossier (Create/Read/Update/Delete)
 */
...
/**
 * Class for DoliFarmFarmDossier
 */
class DoliFarmFarmDossier extends CommonObject
{
    /**
     ...
     */
    public $element = 'dolifarmfarmdossier';
     ...
     */
    public $table_element = 'dolifarm_farmdossier';

    /**
     * @var string      If permission must be checked with hasRight('dolifarm', 'read') and not hasright('dolifarm', 'dolifarmfarmdossier', 'read'), you can uncomment this line
     */
    //public $element_for_permission = 'dolifarm';

    /**
     * @var string      String with name of icon for dolifarmfarmdossier. Must be a 'fa-xxx' fontawesome code (or 'fa-xxx_fa_color_size') or 'dolifarmfarmdossier@dolifarm' if picto is file 'img/object_dolifarmfarmdossier.png'.
     */
    ...
    public $fields = array(
        ...
        "fk_user_validator" => array("type" => "integer:User:user/class/user.class.php", "label" => "Fkuservalidator", "picto" => "user", "enabled" => "1", 'position' => 25, 'notnull' => 0, "visible" => "-1", "css" => "maxwidth500 widthcentpercentminusxx", "csslist" => "tdoverflowmax150",),
        "year_validity" => array("type" => "int", "label" => "Yearvalidity", "enabled" => "1", 'position' => 30, 'notnull' => 1, "visible" => "-1",),
        "date_import" => array("type" => "datetime", "label" => "Dateimport", "enabled" => "1", 'position' => 35, 'notnull' => 0, "visible" => "-1",),
        "status" => array("type" => "int", "label" => "Status", "enabled" => "1", 'position' => 500, 'notnull' => 0, "visible" => "-1",),
        "source_file" => array("type" => "varchar(255)", "label" => "Sourcefile", "enabled" => "1", 'position' => 45, 'notnull' => 0, "visible" => "-1",),
        "note" => array("type" => "text", "label" => "Note", "enabled" => "1", 'position' => 50, 'notnull' => 0, "visible" => "-1",),
        "note_public" => array("type" => "text", "label" => "NotePublic", "enabled" => "1", 'position' => 51, 'notnull' => 0, "visible" => "0", "cssview" => "wordbreak",),
        "note_private" => array("type" => "text", "label" => "NotePrivate", "enabled" => "1", 'position' => 52, 'notnull' => 0, "visible" => "0", "cssview" => "wordbreak",),
        
        "date_creation" => array("type" => "datetime", "label" => "DateCreation", "enabled" => "1", 'position' => 55, 'notnull' => 0, "visible" => "-1",),
        "tms" => array("type" => "timestamp", "label" => "DateModification", "enabled" => "1", 'position' => 60, 'notnull' => 0, "visible" => "-1",),
        
        "fk_user_creat" => array("type" => "integer:User:user/class/user.class.php", "label" => "UserAuthor", "picto" => "user", "enabled" => "1", 'position' => 65, 'notnull' => 0, "visible" => "-1", "css" => "maxwidth500 widthcentpercentminusxx", "csslist" => "tdoverflowmax150",),
        "fk_user_modif" => array("type" => "integer:User:user/class/user.class.php", "label" => "UserModif", "picto" => "user", "enabled" => "1", 'position' => 70, 'notnull' => -1, "visible" => "-2", "css" => "maxwidth500 widthcentpercentminusxx", "csslist" => "tdoverflowmax150",),
    ...
    //  */
    // public $table_element_line = 'dolifarm_dolifarmfarmdossierline';
    ...
    //  */
    // public $fk_element = 'fk_dolifarmfarmdossier';

    // /**
    //  * @var string    Name of subtable class that manage subtable lines
    //  */
    // public $class_element_line = 'DoliFarmFarmDossierline';

    // /**
    //  * @var array    List of child tables. To test if we can delete object.
    //  */
    // protected $childtables = array('mychildtable' => array('name'=>'DoliFarmFarmDossier', 'fk_element'=>'fk_dolifarmfarmdossier'));
    ...
    //  */
    // protected $childtablesoncascade = array('dolifarm_dolifarmfarmdossierdet');

    // /**
    //  * @var DoliFarmFarmDossierLine[]     Array of subtable lines
    //  */
    // public $lines = array();
    ...
    public function __construct(DoliDB $db)
    {
		...
		$this->picto = dolifarm_get_fa_icon('farmdossier');
        ...
        /*if ($user->hasRight('dolifarm', 'dolifarmfarmdossier', 'read')) {
    ...
    public function validate($user, $notrigger = 0)
    {
        ...
        if (!empty($num)) {
            ...
            if (!empty($this->fields['fk_user_valid'])) {
                $sql .= ", fk_user_validator = ".((int) $user->id); // Fixed: SQL field name
            }
        ...
        if (!$error) {
            ...
            if (preg_match('/^[\(]?PROV/i', $this->ref)) {
                ...
                $sql = 'UPDATE '.$this->db->prefix()."ecm_files set filename = CONCAT('".$this->db->escape($this->newref)."', SUBSTR(filename, ".(strlen($this->ref) + 1).")), filepath = 'dolifarmfarmdossier/".$this->db->escape($this->newref)."'";
                $sql .= " WHERE filename LIKE '".$this->db->escape($this->ref)."%' AND filepath = 'dolifarmfarmdossier/".$this->db->escape($this->ref)."' and entity = ".$conf->entity;
                $resql = $this->db->query($sql);
                if (!$resql) {
                    $error++;
                    $this->error = $this->db->lasterror();
                }
                $sql = 'UPDATE '.$this->db->prefix()."ecm_files set filepath = 'dolifarmfarmdossier/".$this->db->escape($this->newref)."'";
                $sql .= " WHERE filepath = 'dolifarmfarmdossier/".$this->db->escape($this->ref)."' and entity = ".$conf->entity;
                $resql = $this->db->query($sql);
                if (!$resql) {
                    $error++;
                    $this->error = $this->db->lasterror();
                ...
                $dirsource = $conf->dolifarm->dir_output.'/dolifarmfarmdossier/'.$oldref;
                $dirdest = $conf->dolifarm->dir_output.'/dolifarmfarmdossier/'.$newref;
                if (!$error && file_exists($dirsource)) {
                    ...
                    if (@rename($dirsource, $dirdest)) {
                        ...
                        $listoffiles = dol_dir_list($conf->dolifarm->dir_output.'/dolifarmfarmdossier/'.$newref, 'files', 1, '^'.preg_quote($oldref, '/'));
    ...
    public function getTooltipContentArray($params)
    {
        ...
        if (getDolGlobalInt('MAIN_OPTIMIZEFORTEXTBROWSER')) {
            return ['optimize' => $langs->trans("ShowDoliFarmFarmDossier")];
        }
        $datas['picto'] = img_picto('', $this->picto).' <u>'.$langs->trans("DoliFarmFarmDossier").'</u>';
    ...
    public function getNomUrl($withpicto = 0, $option = '', $notooltip = 0, $morecss = '', $save_lastsearch_value = -1)
    {
        ...
        $baseurl = dol_buildpath('/dolifarm/dolifarmfarmdossier_card.php', 1);
        $query = ['id' => $this->id];
        if ($option !== 'nolink') {
        ...
        if (empty($notooltip)) {
            if (getDolGlobalInt('MAIN_OPTIMIZEFORTEXTBROWSER')) {
                $label = $langs->trans("ShowDoliFarmFarmDossier");
    ...
    {
        $sql = "SELECT t.rowid, t.date_creation as datec, t.tms as datem, t.fk_user_creat, t.fk_user_modif, t.import_key";
        $sql .= " FROM ".$this->db->prefix().$this->table_element." as t";
     ...
     *
     * @return DoliFarmFarmDossierLine[]|int      array of lines if OK, <0 if KO
     */
    public function getLinesArray()
    {
        $this->lines = array();

        $objectline = new DoliFarmFarmDossierLine($this->db);
        $result = $objectline->fetchAll('ASC', 'position', 0, 0, '(fk_dolifarmfarmdossier:=:'.((int) $this->id).')');
    ...
    public function getNextNumRef()
    {
        ...
        if (!getDolGlobalString('DOLIFARM_MYOBJECT_ADDON')) {
            $conf->global->DOLIFARM_MYOBJECT_ADDON = 'mod_dolifarmfarmdossier_standard';
        }
        ...
        if (getDolGlobalString('DOLIFARM_MYOBJECT_ADDON')) {
            ...
            if (class_exists($classname)) {
                $obj = new $classname();
                '@phan-var-force ModeleNumRefDoliFarmFarmDossier $obj';
...
/**
 * Class DoliFarmFarmDossierLine
 */
class DoliFarmFarmDossierLine extends CommonObjectLine
{
    /**
     * @var string ID of module.
     */
     ...
     * @var string ID to identify managed object.
     */
    public $element = 'dolifarmfarmdossierline';

    /**
     * @var string Name of table without prefix where object is stored.
     */
    public $table_element = 'dolifarm_farmdossier_det'; // Placeholder
