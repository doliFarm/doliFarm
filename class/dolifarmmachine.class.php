...
/**
 * \file        class/dolifarmmachine.class.php
 * \ingroup     dolifarm
 * \brief       This file is a CRUD class file for DoliFarmMachine (Create/Read/Update/Delete)
 */
...
/**
 * Class for DoliFarmMachine
 */
class DoliFarmMachine extends CommonObject
{
    /**
     ...
     */
    public $element = 'dolifarmmachine';
     ...
     */
    public $table_element = 'dolifarm_machine'; // Renamed to match cleaner SQL schema

    /**
     * @var string      If permission must be checked with hasRight('dolifarm', 'read') and not hasright('dolifarm', 'dolifarmmachine', 'read'), you can uncomment this line
     */
    //public $element_for_permission = 'dolifarm';

    /**
     * @var string      String with name of icon for dolifarmmachine. Must be a 'fa-xxx' fontawesome code (or 'fa-xxx_fa_color_size') or 'dolifarmmachine@dolifarm' if picto is file 'img/object_dolifarmmachine.png'.
     */
    ...
    public $fields = array(
        ...
        // Linked to Dictionary: llx_dolifarm_c_machinetype. Storing CODE.
        "fk_type" => array("type" => "sellist:dolifarm_c_machinetype:label:code::active=1", "label" => "MachineType", "enabled" => "1", 'position' => 35, 'notnull' => 0, "visible" => "-1", "css" => "maxwidth500 widthcentpercentminusxx",),
        
        "power_kw" => array("type" => "double(10,2)", "label" => "Powerkw", "enabled" => "1", 'position' => 40, 'notnull' => 0, "visible" => "-1",),
        
        // Linked to Dictionary: llx_dolifarm_c_fueltype. Storing CODE.
        "fuel_type" => array("type" => "sellist:dolifarm_c_fueltype:label:code::active=1", "label" => "FuelType", "enabled" => "1", 'position' => 45, 'notnull' => 0, "visible" => "-1",),
        
        "year_registration" => array("type" => "int", "label" => "Yearregistration", "enabled" => "1", 'position' => 50, 'notnull' => 0, "visible" => "-1",),
        
        "status" => array("type" => "int", "label" => "Status", "enabled" => "1", 'position' => 52, 'notnull' => 0, "visible" => "-1",),
        
        "datec" => array("type" => "datetime", "label" => "DateCreation", "enabled" => "1", 'position' => 55, 'notnull' => 0, "visible" => "-1",),
        "date_valid" => array("type" => "datetime", "label" => "DateValidation", "enabled" => "1", 'position' => 56, 'notnull' => 0, "visible" => "-1",),
        "tms" => array("type" => "timestamp", "label" => "DateModification", "enabled" => "1", 'position' => 57, 'notnull' => 0, "visible" => "-1",),
        
        // Standard Dolibarr tracking
        "fk_user_creat" => array("type" => "integer:User:user/class/user.class.php", "label" => "UserAuthor", "picto" => "user", "enabled" => "1", 'position' => 60, 'notnull' => 0, "visible" => "-1", "css" => "maxwidth500 widthcentpercentminusxx", "csslist" => "tdoverflowmax150",),
    ...
    //  */
    // public $table_element_line = 'dolifarm_dolifarmmachineline';
    ...
    //  */
    // public $fk_element = 'fk_dolifarmmachine';

    // /**
    //  * @var string    Name of subtable class that manage subtable lines
    //  */
    // public $class_element_line = 'DoliFarmMachineline';

    // /**
    //  * @var array    List of child tables. To test if we can delete object.
    //  */
    // protected $childtables = array('mychildtable' => array('name'=>'DoliFarmMachine', 'fk_element'=>'fk_dolifarmmachine'));
    ...
    //  */
    // protected $childtablesoncascade = array('dolifarm_dolifarmmachinedet');

    // /**
    //  * @var DoliFarmMachineLine[]     Array of subtable lines
    //  */
    // public $lines = array();
    ...
    public function __construct(DoliDB $db)
    {
		...
		$this->picto = dolifarm_get_fa_icon('machine');
        ...
        /*if ($user->hasRight('dolifarm', 'dolifarmmachine', 'read')) {
    ...
    public function validate($user, $notrigger = 0)
    {
        ...
        if (!$error) {
            ...
            if (preg_match('/^[\(]?PROV/i', $this->ref)) {
                ...
                $sql = 'UPDATE '.$this->db->prefix()."ecm_files set filename = CONCAT('".$this->db->escape($this->newref)."', SUBSTR(filename, ".(strlen($this->ref) + 1).")), filepath = 'dolifarmmachine/".$this->db->escape($this->newref)."'";
                $sql .= " WHERE filename LIKE '".$this->db->escape($this->ref)."%' AND filepath = 'dolifarmmachine/".$this->db->escape($this->ref)."' and entity = ".$conf->entity;
                $resql = $this->db->query($sql);
                if (!$resql) {
                    $error++;
                    $this->error = $this->db->lasterror();
                }
                $sql = 'UPDATE '.$this->db->prefix()."ecm_files set filepath = 'dolifarmmachine/".$this->db->escape($this->newref)."'";
                $sql .= " WHERE filepath = 'dolifarmmachine/".$this->db->escape($this->ref)."' and entity = ".$conf->entity;
                $resql = $this->db->query($sql);
                if (!$resql) {
                    $error++;
                    $this->error = $this->db->lasterror();
                ...
                $dirsource = $conf->dolifarm->dir_output.'/dolifarmmachine/'.$oldref;
                $dirdest = $conf->dolifarm->dir_output.'/dolifarmmachine/'.$newref;
                if (!$error && file_exists($dirsource)) {
                    ...
                    if (@rename($dirsource, $dirdest)) {
                        ...
                        $listoffiles = dol_dir_list($conf->dolifarm->dir_output.'/dolifarmmachine/'.$newref, 'files', 1, '^'.preg_quote($oldref, '/'));
    ...
    public function getTooltipContentArray($params)
    {
        ...
        if (getDolGlobalInt('MAIN_OPTIMIZEFORTEXTBROWSER')) {
            return ['optimize' => $langs->trans("ShowDoliFarmMachine")];
        }
        $datas['picto'] = img_picto('', $this->picto).' <u>'.$langs->trans("DoliFarmMachine").'</u>';
    ...
    public function getNomUrl($withpicto = 0, $option = '', $notooltip = 0, $morecss = '', $save_lastsearch_value = -1)
    {
        ...
        $baseurl = dol_buildpath('/dolifarm/dolifarmmachine_card.php', 1);
        $query = ['id' => $this->id];
        if ($option !== 'nolink') {
        ...
        if (empty($notooltip)) {
            if (getDolGlobalInt('MAIN_OPTIMIZEFORTEXTBROWSER')) {
                $label = $langs->trans("ShowDoliFarmMachine");
     ...
     *
     * @return DoliFarmMachineLine[]|int      array of lines if OK, <0 if KO
     */
    public function getLinesArray()
    {
        $this->lines = array();

        $objectline = new DoliFarmMachineLine($this->db);
        $result = $objectline->fetchAll('ASC', 'position', 0, 0, '(fk_dolifarmmachine:=:'.((int) $this->id).')');
    ...
    public function getNextNumRef()
    {
        ...
        if (!getDolGlobalString('DOLIFARM_MYOBJECT_ADDON')) {
            $conf->global->DOLIFARM_MYOBJECT_ADDON = 'mod_dolifarmmachine_standard';
        }
        ...
        if (getDolGlobalString('DOLIFARM_MYOBJECT_ADDON')) {
            ...
            if (class_exists($classname)) {
                $obj = new $classname();
                '@phan-var-force ModeleNumRefDoliFarmMachine $obj';
    ...
    public function generateDocument($modele, $outputlangs, $hidedetails = 0, $hidedesc = 0, $hideref = 0, $moreparams = null)
    {
        ...
        if (!dol_strlen($modele)) {
            if (!empty($this->model_pdf)) {
                ...
                $modele = getDolGlobalString('MYOBJECT_ADDON_PDF', 'standard_dolifarmmachine');
            }
        }
...
/**
 * Class DoliFarmMachineLine. You can also remove this and generate a CRUD class for lines objects.
 */
class DoliFarmMachineLine extends CommonObjectLine
{
    // To complete with content of an object DoliFarmMachineLine
    // We should have a field rowid, fk_dolifarmmachine and position

    /**
     * To overload
     * @see CommonObjectLine
     */
    public $parent_element = '';        // Example: '' or 'dolifarmmachine'

    /**
     * To overload
     * @see CommonObjectLine
     */
    public $fk_parent_attribute = '';   // Example: '' or 'fk_dolifarmmachine'
