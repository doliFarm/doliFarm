...
/**
 * \file        class/dolifarmagrodrug.class.php
 * \ingroup     dolifarm
 * \brief       This file is a CRUD class file for DoliFarmAgroDrug (Create/Read/Update/Delete)
 */
...
/**
 * Class for DoliFarmAgroDrug
 */
class DoliFarmAgroDrug extends CommonObject
{
	/**
	 ...
	 */
	public $element = 'dolifarmagrodrug';

	/**
	 * @var string		Prefix to check for any trigger code of any business class to prevent bad value for trigger code.
	 * @see CommonTrigger::call_trigger()
	 */
	public $TRIGGER_PREFIX = 'DOLIFARM_MYOBJECT';	// Will be used to build trgiger keys 'DOLIFARM_MYOBJECT_MODIFY', ...

	/**
	 * @var string 		Name of table without prefix where object is stored. This is also the key used for extrafields management (so extrafields know the link to the parent table).
	 */
	public $table_element = 'dolifarm_agrodrug';

	/**
	 * @var string 		If permission must be checked with hasRight('dolifarm', 'read') and not hasright('dolifarm', 'dolifarmagrodrug', 'read'), you can uncomment this line
	 */
	//public $element_for_permission = 'dolifarm';

	/**
	 * @var string 		String with name of icon for dolifarmagrodrug. Must be a 'fa-xxx' fontawesome code (or 'fa-xxx_fa_color_size') or 'dolifarmagrodrug@dolifarm' if picto is file 'img/object_dolifarmagrodrug.png'.
	 */
	...
	public $fields = array(
		...
		"fk_product" => array("type" => "integer:Product:product/class/product.class.php:1", "label" => "Fkproduct", "picto" => "product", "enabled" => "1", 'position' => 30, 'notnull' => 1, "visible" => "-1", "css" => "maxwidth500 widthcentpercentminusxx", "csslist" => "tdoverflowmax150",),
		"reg_number" => array("type" => "varchar(64)", "label" => "Regnumber", "enabled" => "1", 'position' => 35, 'notnull' => 0, "visible" => "-1",),
		"reg_date" => array("type" => "date", "label" => "Regdate", "enabled" => "1", 'position' => 40, 'notnull' => 0, "visible" => "-1",),
		"auth_expiration_date" => array("type" => "date", "label" => "Authexpirationdate", "enabled" => "1", 'position' => 45, 'notnull' => 0, "visible" => "-1",),
		"manufacturer" => array("type" => "varchar(255)", "label" => "Manufacturer", "enabled" => "1", 'position' => 50, 'notnull' => 0, "visible" => "-1",),
		"active_substance" => array("type" => "text", "label" => "Activesubstance", "enabled" => "1", 'position' => 55, 'notnull' => 0, "visible" => "-1",),
		"content_percentage" => array("type" => "varchar(64)", "label" => "Contentpercentage", "enabled" => "1", 'position' => 60, 'notnull' => 0, "visible" => "-1",),
		"formulation" => array("type" => "varchar(128)", "label" => "Formulation", "enabled" => "1", 'position' => 65, 'notnull' => 0, "visible" => "-1",),
		"hazard_classification" => array("type" => "varchar(128)", "label" => "Hazardclassification", "enabled" => "1", 'position' => 70, 'notnull' => 0, "visible" => "-1",),
		"safety_days_default" => array("type" => "int", "label" => "Safetydaysdefault", "enabled" => "1", 'position' => 75, 'notnull' => 0, "visible" => "-1",),
		"max_dose_ha" => array("type" => "double(24,8)", "label" => "Maxdoseha", "enabled" => "1", 'position' => 80, 'notnull' => 0, "visible" => "-1",),
		"note_public" => array("type" => "text", "label" => "NotePrivate", "enabled" => "1", 'position' => 85, 'notnull' => 0, "visible" => "0", "cssview" => "wordbreak",),
		"note_private" => array("type" => "text", "label" => "NotePublic", "enabled" => "1", 'position' => 90, 'notnull' => 0, "visible" => "0", "cssview" => "wordbreak",),
		"status" => array("type" => "int", "label" => "Status", "enabled" => "1", 'position' => 500, 'notnull' => 0, "visible" => "-1",),
		"date_creation" => array("type" => "datetime", "label" => "DateCreation", "enabled" => "1", 'position' => 100, 'notnull' => 0, "visible" => "-1",),
		"tms" => array("type" => "timestamp", "label" => "DateModification", "enabled" => "1", 'position' => 105, 'notnull' => 0, "visible" => "-1",),
	...
	public $reg_number;
	public $reg_date;
	public $auth_expiration_date;
	public $manufacturer;
	public $active_substance;
	public $content_percentage;
	public $formulation;
	public $hazard_classification;
	public $safety_days_default;
	public $max_dose_ha;
	public $note_public;
	public $note_private;
	public $status;
	...
	//  */
	// public $table_element_line = 'dolifarm_agrodrugline';
	...
	//  */
	// public $fk_element = 'fk_dolifarmagrodrug';

	// /**
	//  * @var string    Name of subtable class that manage subtable lines
	//  */
	// public $class_element_line = 'DoliFarmAgroDrugline';

	// /**
	//  * @var array	List of child tables. To test if we can delete object.
	//  */
	// protected $childtables = array('mychildtable' => array('name'=>'DoliFarmAgroDrug', 'fk_element'=>'fk_dolifarmagrodrug'));
	...
	//  */
	// protected $childtablesoncascade = array('dolifarm_agrodrugdet');

	// /**
	//  * @var DoliFarmAgroDrugLine[]     Array of subtable lines
	//  */
	// public $lines = array();
	...
	public function __construct(DoliDB $db)
	{
		...
		$this->picto = dolifarm_get_fa_icon('agrodrug');
		...
		/*if ($user->hasRight('dolifarm', 'dolifarmagrodrug', 'read')) {
	...
	public function validate($user, $notrigger = 0)
	{
		...
		/* if (! ((!getDolGlobalInt('MAIN_USE_ADVANCED_PERMS') && $user->hasRight('dolifarm', 'dolifarmagrodrug', 'write'))
		 || (getDolGlobalInt('MAIN_USE_ADVANCED_PERMS') && $user->hasRight('dolifarm', 'dolifarmagrodrug_advance', 'validate')))
		 {
		...
		if (!$error) {
			...
			if (preg_match('/^[\(]?PROV/i', $this->ref)) {
				...
				$sql = 'UPDATE '.$this->db->prefix()."ecm_files set filename = CONCAT('".$this->db->escape($this->newref)."', SUBSTR(filename, ".(strlen($this->ref) + 1).")), filepath = 'dolifarmagrodrug/".$this->db->escape($this->newref)."'";
				$sql .= " WHERE filename LIKE '".$this->db->escape($this->ref)."%' AND filepath = 'dolifarmagrodrug/".$this->db->escape($this->ref)."' and entity = ".$conf->entity;
				$resql = $this->db->query($sql);
				if (!$resql) {
					$error++;
					$this->error = $this->db->lasterror();
				}
				$sql = 'UPDATE '.$this->db->prefix()."ecm_files set filepath = 'dolifarmagrodrug/".$this->db->escape($this->newref)."'";
				$sql .= " WHERE filepath = 'dolifarmagrodrug/".$this->db->escape($this->ref)."' and entity = ".$conf->entity;
				$resql = $this->db->query($sql);
				if (!$resql) {
					$error++;
					$this->error = $this->db->lasterror();
				...
				$dirsource = $conf->dolifarm->dir_output.'/dolifarmagrodrug/'.$oldref;
				$dirdest = $conf->dolifarm->dir_output.'/dolifarmagrodrug/'.$newref;
				if (!$error && file_exists($dirsource)) {
					...
					if (@rename($dirsource, $dirdest)) {
						...
						$listoffiles = dol_dir_list($conf->dolifarm->dir_output.'/dolifarmagrodrug/'.$newref, 'files', 1, '^'.preg_quote($oldref, '/'));
	...
	public function getTooltipContentArray($params)
	{
		...
		if (getDolGlobalInt('MAIN_OPTIMIZEFORTEXTBROWSER')) {
			return ['optimize' => $langs->trans("ShowDoliFarmAgroDrug")];
		}
		$datas['picto'] = img_picto('', $this->picto).' <u>'.$langs->trans("DoliFarmAgroDrug").'</u>';
	...
	public function getNomUrl($withpicto = 0, $option = '', $notooltip = 0, $morecss = '', $save_lastsearch_value = -1)
	{
		...
		$baseurl = dol_buildpath('/dolifarm/dolifarmagrodrug_card.php', 1);
		$query = ['id' => $this->id];
		if ($option !== 'nolink') {
		...
		if (empty($notooltip)) {
			if (getDolGlobalInt('MAIN_OPTIMIZEFORTEXTBROWSER')) {
				$label = $langs->trans("ShowDoliFarmAgroDrug");
	...
	public function getLinesArray()
	{
		...
		$objectline = new DoliFarmAgroDrugLine($this->db);
		$result = $objectline->fetchAll('ASC', 'position', 0, 0, '(fk_dolifarmagrodrug:=:'.((int) $this->id).')');
	...
	public function getNextNumRef()
	{
		...
		if (!getDolGlobalString('DOLIFARM_MYOBJECT_ADDON')) {
			$conf->global->DOLIFARM_MYOBJECT_ADDON = 'mod_dolifarmagrodrug_standard';
		}
		...
		if (getDolGlobalString('DOLIFARM_MYOBJECT_ADDON')) {
			...
			if (class_exists($classname)) {
				$obj = new $classname();
				'@phan-var-force ModeleNumRefDoliFarmAgroDrug $obj';
	...
	public function generateDocument($modele, $outputlangs, $hidedetails = 0, $hidedesc = 0, $hideref = 0, $moreparams = null)
	{
		...
		if (!dol_strlen($modele)) {
			if (!empty($this->model_pdf)) {
				...
				$modele = getDolGlobalString('MYOBJECT_ADDON_PDF', 'standard_dolifarmagrodrug');
			}
		}

		$modelpath = "core/modules/dolifarm/doc/";
...
/**
 * Class DoliFarmAgroDrugLine. You can also remove this and generate a CRUD class for lines objects.
 */
class DoliFarmAgroDrugLine extends CommonObjectLine
{
	// To complete with content of an object DoliFarmAgroDrugLine
	// We should have a field rowid, fk_dolifarmagrodrug and position

	/**
	 * To overload
	 * @see CommonObjectLine
	 */
	public $parent_element = '';		// Example: '' or 'dolifarmagrodrug'

	/**
	 * To overload
	 * @see CommonObjectLine
	 */
	public $fk_parent_attribute = '';	// Example: '' or 'fk_dolifarmagrodrug'
