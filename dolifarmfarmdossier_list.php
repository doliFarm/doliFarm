...
/**
 *   	\file       dolifarmfarmdossier_list.php
 *		\ingroup    dolifarm
 *		\brief      List page for dolifarmfarmdossier
 */
...
include_once __DIR__.'/class/dolifarmfarmdossier.class.php';
require_once DOL_DOCUMENT_ROOT.'/core/lib/files.lib.php';
// for other modules
//dol_include_once('/othermodule/class/otherobject.class.php');

// Load translation files required by the page
$langs->loadLangs(array("dolifarm@dolifarm", "other"));
...
$enablepermissioncheck = getDolGlobalInt('DOLIFARM_ENABLE_PERMISSION_CHECK');
if ($enablepermissioncheck) {
	$permissiontoread = $user->hasRight('dolifarm', 'dolifarmfarmdossier', 'read');
	$permissiontoadd = $user->hasRight('dolifarm', 'dolifarmfarmdossier', 'write');
	$permissiontodelete = $user->hasRight('dolifarm', 'dolifarmfarmdossier', 'delete');
} else {
	$permissiontoread = 1;
	$permissiontoadd = 1;
...
if (!isModEnabled("dolifarm")) {
	accessforbidden('Module dolifarm not enabled');
}
if (!$permissiontoread) {
	accessforbidden();
}
...
// ... altri codici di azione esistenti ...

// -------------------------------------------------------------------------
// ACTION: IMPORTAZIONE XML
// -------------------------------------------------------------------------
if ($action == 'import_xml' && $permissiontoadd) {
    $error = 0;
    
    // 1. Verifica caricamento file
    if (empty($_FILES['file_import']['name'])) {
        setEventMessages($langs->trans("ErrorFileNotSelected"), null, 'errors');
        $error++;
    }

    if (!$error) {
        // 2. Preparazione Cartella Temporanea
        $upload_dir = $conf->dolifarm->dir_temp;
        ...
        $dest_file = $upload_dir . '/' . dol_sanitizeFileName($file_name);

        // 3. Spostamento sicuro del file (Funzione Core Dolibarr)
        if (dol_move_uploaded_file($file_tmp, $dest_file, 1)) {
            
            // 4. ESECUZIONE IMPORTER
            dol_include_once('/dolifarm/class/dolifarmimporter.class.php');
            
            if (class_exists('DoliFarmImporter')) {
                $importer = new DoliFarmImporter($db, $user);
                $res = $importer->importFromXML($dest_file);

                if ($res > 0) {
                    setEventMessages("Importazione completata con successo! Creato Fascicolo ID: ".$res, null, 'mesgs');
                } else {
                    setEventMessages("Errore Importazione: " . $importer->error, $importer->errors, 'errors');
                }
                
                // Pulizia file temp
                @unlink($dest_file);
            } else {
                setEventMessages("Errore Critico: Classe DoliFarmImporter non trovata.", null, 'errors');
            }
...
if (empty($reshook)) {
	...
	// Mass actions
	$objectclass = 'DoliFarmFarmDossier';
	$objectlabel = 'DoliFarmFarmDossier';
	$uploaddir = $conf->dolifarm->dir_output;
...
$title = $langs->trans("DoliFarmFarmDossiers");
//$help_url = "EN:Module_DoliFarmFarmDossier|FR:Module_DoliFarmFarmDossier_FR|ES:Módulo_DoliFarmFarmDossier";
$help_url = '';
$morejs = array();
$morecss = array();
...
if ($num == 1 && getDolGlobalInt('MAIN_SEARCH_DIRECT_OPEN_IF_ONLY_ONE') && $search_all && !$page) {
	...
	$id = $obj->rowid;
	header("Location: ".dol_buildpath('/dolifarm/dolifarmfarmdossier_card.php', 1).'?id='.((int) $id));
	exit;
}
...
llxHeader('', $title, $help_url, '', 0, 0, $morejs, $morecss, '', 'mod-dolifarm page-list bodyforlist');	// Can use also classforhorizontalscrolloftabs instead of bodyforlist for a horizontal scroll in the table instead of page
...
// -------------------------------------------------------------------------
// BOX IMPORTAZIONE (Visibile solo se si hanno permessi di scrittura)
// -------------------------------------------------------------------------
if ($permissiontoadd) {
    print '<div class="box">';
    print '<form enctype="multipart/form-data" action="'.$_SERVER["PHP_SELF"].'" method="post">';
    print '<input type="hidden" name="token" value="'.newToken().'">';
    print '<input type="hidden" name="action" value="import_xml">';
    
    print '<table class="noborder" width="100%">';
    print '<tr class="liste_titre"><td colspan="3">Importazione Rapida Fascicolo (XML)</td></tr>';
    
    print '<tr class="oddeven">';
    print '<td width="150">Seleziona File XML:</td>';
    print '<td><input type="file" name="file_import" accept=".xml"></td>';
    print '<td align="right">';
    print '<input type="submit" class="button" value="Importa Fascicolo">';
    print '</td>';
    print '</tr>';
...
// -------------------------------------------------------------------------
// BOX IMPORTAZIONE (Visibile solo se si hanno permessi di scrittura)  END
// -------------------------------------------------------------------------
...
$newcardbutton .= dolGetButtonTitle($langs->trans('New'), '', 'fa fa-plus-circle', dol_buildpath('/dolifarm/dolifarmfarmdossier_card.php', 1).'?action=create&backtopage='.urlencode($_SERVER['PHP_SELF']), '', $permissiontoadd);

print_barre_liste($title, $page, $_SERVER["PHP_SELF"], $param, $sortfield, $sortorder, $massactionbutton, $num, $nbtotalofrecords, $object->picto, 0, $newcardbutton, '', $limit, 0, 0, 1);

// Add code for pre mass action (confirmation or email presend form)
$topicmail = "SendDoliFarmFarmDossierRef";
$modelmail = "dolifarmfarmdossier";
$objecttmp = new DoliFarmFarmDossier($db);
