...
/**
 *	\file       htdocs/core/modules/dolifarm/doc/doc_generic_dolifarmmachine_odt.modules.php
 *	\ingroup    dolifarm
 *	\brief      File of class to build ODT documents for dolifarmmachines
 */

dol_include_once('/dolifarm/core/modules/dolifarm/modules_dolifarmmachine.php');
 ...
 */
class doc_generic_dolifarmmachine_odt extends ModelePDFDoliFarmMachine
{
	/**
	 * Issuer
	 * @var Societe
	...
	public function info($langs)
	{
		...
		if ($nbofiles) {
			...
			foreach ($listoffiles as $file) {
				$texte .= '- '.$file['name'].' <a href="'.DOL_URL_ROOT.'/document.php?modulepart=doctemplates&file=dolifarm_dolifarmmachine/'.urlencode(basename($file['name'])).'">'.img_picto('', 'listlight').'</a>';
	 ...
	 *
	 *  @param	DoliFarmMachine	$object				Source object to generate document from
	...
	public function write_file($object, $outputlangs, $srctemplatepath = '', $hidedetails = 0, $hidedesc = 0, $hideref = 0)
	{
		...
		if ($conf->dolifarm->dir_output) {
			...
			if (!is_object($object)) {
				$id = $object;
				$object = new DoliFarmMachine($this->db);
