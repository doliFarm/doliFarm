...
/**
 *   	\file       dolifarmcrops_list.php
 *		\ingroup    dolifarm
 *		\brief      List page for dolifarmcrops
 */
...
include_once __DIR__.'/class/dolifarmcrops.class.php';
// for other modules
...
if ($enablepermissioncheck) {
	...
	$permissiontoadd = $user->hasRight('dolifarm', 'dolifarmcrops', 'write');
	$permissiontodelete = $user->hasRight('dolifarm', 'dolifarmcrops', 'delete');
} else {
	$permissiontoread = 1;
	$permissiontoadd = 1;
...
if (empty($reshook)) {
	...
	// Mass actions
	$objectclass = 'DoliFarmCrops';
	$objectlabel = 'DoliFarmCrops';
...
$title = $langs->trans("DoliFarmCropss");
//$help_url = "EN:Module_DoliFarmCrops|FR:Module_DoliFarmCrops_FR|ES:Módulo_DoliFarmCrops";
$help_url = '';
$morejs = array();
$morecss = array();
...
if ($num == 1 && getDolGlobalInt('MAIN_SEARCH_DIRECT_OPEN_IF_ONLY_ONE') && $search_all && !$page) {
	...
	$id = $obj->rowid;
	header("Location: ".dol_buildpath('/dolifarm/dolifarmcrops_card.php', 1).'?id='.((int) $id));
	exit;
}
...
$newcardbutton .= dolGetButtonTitle($langs->trans('New'), '', 'fa fa-plus-circle', dol_buildpath('/dolifarm/dolifarmcrops_card.php', 1).'?action=create&backtopage='.urlencode($_SERVER['PHP_SELF']), '', $permissiontoadd);

print_barre_liste($title, $page, $_SERVER["PHP_SELF"], $param, $sortfield, $sortorder, $massactionbutton, $num, $nbtotalofrecords, $object->picto, 0, $newcardbutton, '', $limit, 0, 0, 1);

// Add code for pre mass action (confirmation or email presend form)
$topicmail = "SendDoliFarmCropsRef";
$modelmail = "dolifarmcrops";
$objecttmp = new DoliFarmCrops($db);
